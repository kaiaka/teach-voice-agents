...
class RealtimeClient:
    def __init__(self, loop=None, log_event=None, speaker=None, audio_user_path=None, ai_audio_logger=None):
        self.loop = loop
        self.log_event = log_event or (lambda *a, **kw: None)
    ...
    def _on_message(self, ws, raw, *_):
        ...
        if typ in ("response.created", "response.started"):
            ...
            if rid and rid != self._current_response_id:
                ...
                self._drop_audio_until_new_response = False
                self.log_event("message_event",source="realtime_api",value=typ,extra=json.dumps({"rid": rid}))
                if self.speech_visualizer:
        ...
        if typ == "response.audio.delta":
            ...
            if rid and rid not in self._first_audio_seen_for_rid:
                ...
                self.log_event("message_event", source="realtime_api", value="response.audio.first_delta", extra=json.dumps({"rid": rid}))
                if self.ai_audio_logger:
    ...
    def _on_error(self, ws, error):
        ...
        self.log_event("message_event",source="websocket",value="ws_error",extra=str(error))

    def _on_close(self, _ws, code, reason):
        print("WebSocket closed", code, reason)
        self.log_event("message_event", source="websocket", value="ws_close", extra=json.dumps({"code": code, "reason": reason}))
        if self.speech_visualizer:
            self.speech_visualizer.stop_speaking()
